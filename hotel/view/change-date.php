...
<body data-bs-spy="scroll" data-bs-offset="120" data-bs-target="#mainNav">
                            ...
                            <div class="col-lg-6">
                                <div class="custom_select">
                                    <select class="wide" id="kamarDropdown">
                                        ...
                                        WHERE checkout.id_RS IS NULL AND bed.id_bed=".$test['id_bed']." OR history.id_C IS NOT NULL AND bed.id_bed=".$test['id_bed'];
                                        $result = mysqli_query($koneksi, $query);
                                        if (!empty($test['id_k'])) {   
    ...
    $("#rooms_booking").change(function() {
        ...
        LEFT JOIN checkout ON checkout.id_RS = reserfasi.id_RS
        LEFT JOIN history ON checkout.id_C = history.id_C
        WHERE checkout.id_RS IS NULL OR history.id_C IS NOT NULL;";
        $result = mysqli_query($koneksi, $query);
        $first = true;
        $jenisSelec = "";
        if (!empty($_GET['id_1'])) {
